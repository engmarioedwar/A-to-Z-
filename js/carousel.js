/* ═══════════════════════════════════════════════════════════
   AIDS SOC Platform  —  carousel.js  (v2 — Mobile Optimized)
   Fixes:
...
function openExpanded(idx) {
  ...
  /* Stop ALL three mini-previews the moment we go full-screen on any block —
     they are invisible behind the overlay anyway, so there is no reason
     for them to keep burning a render loop while a much heavier expanded
     view is also running. They resume automatically on close. */
  if (window.pauseMiniPreviews) pauseMiniPreviews();
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (idx === 1 && window.startPipelineDemo)     startPipelineDemo();
          if (idx === 2 && window.startCopilotDemo)      startCopilotDemo();
        }, dashDelay);
      }, revealDelay);
    });
  }, { once: true });
...
function closeExpanded() {
  ...
  if (window.stopPipelineDemo) stopPipelineDemo();
  if (window.stopCopilotDemo)  stopCopilotDemo();
  ...
  setTimeout(() => {
    ...
    overlay.addEventListener('animationend', () => {
      ...
      isExpanded = false;
      startAutoRotate();
      /* Mini-previews come back to life only once we're back on the carousel. */
      if (window.resumeMiniPreviews) resumeMiniPreviews();
    }, { once: true });
  }, IS_MOB ? 60 : 120);
}

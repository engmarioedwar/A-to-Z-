/* ═══════════════════════════════════════════════════════════
   AIDS SOC Platform  —  soc_dashboard.js  (v2 — Mobile Optimized)
   Performance fixes:
...
/* ── GLOBAL MINI-PREVIEW COORDINATOR ──
   Called by carousel.js whenever ANY block goes fullscreen (expanded
   view) or comes back to the carousel. This guarantees that at most
   ONE set of animation loops is ever running at the same time on the
   whole page — either the 3 small carousel previews, OR the single
   expanded view's own (heavier) canvases, never both together. */
function pauseMiniPreviews() {
  miniMapPaused = true;
  if (miniMapAnimId) { cancelAnimationFrame(miniMapAnimId); miniMapAnimId = null; }
  if (window.pausePipelineMini) pausePipelineMini();
  if (window.pauseCopilotMini)  pauseCopilotMini();
}
window.pauseMiniPreviews = pauseMiniPreviews;

function resumeMiniPreviews() {
  if (!IS_LOW_END) initMiniMap();   /* resumes if already created, else creates it */
  if (window.resumePipelineMini) resumePipelineMini();
  if (window.resumeCopilotMini)  resumeCopilotMini();
}
window.resumeMiniPreviews = resumeMiniPreviews;
...
function initMiniMap() {
  ...
  /* If already initialized once (mini-canvases are init'd only once,
     same as the rest of the carousel previews), just resume drawing. */
  if (_miniMapDraw) {
    miniMapPaused = false;
    miniMapAnimId = requestAnimationFrame(_miniMapDraw);
    return;
  }
  ...
  let frameCount = 0;
  function draw() {
    if (miniMapPaused) return;
    frameCount++;
    /* Skip every other frame on mobile */
    if (IS_MOBILE && frameCount % 2 !== 0) {
      miniMapAnimId = requestAnimationFrame(draw);
      return;
    }
...
(function initPlaceholderCanvases() {
  ...
  /* mini-pipe canvas is now owned entirely by pipeline.js (initPipelineMini).
     mini-intel no longer exists — block 2 is now AI Copilot, owned by copilot.js. */
})();